...
(function () {
  ...
  /* ═════════════════════════════════════════════════════════════
     ...
     ═════════════════════════════════════════════════════════════ */
  /* ═════════════════════════════════════════════════════════════
     LÊ O VALOR VINDO DA PÁGINA DO IMÓVEL
     Ex: simulador/index.html?valor=1490000&imovel=Cielo%20Vitta
     Se o parâmetro não vier, nada muda — o simulador abre normal.
     ═════════════════════════════════════════════════════════════ */
  function aplicarValorDaUrl() {
    const params = new URLSearchParams(location.search);
    const bruto = params.get("valor");
    if (!bruto) return;

    const valor = Number(String(bruto).replace(/\D/g, ""));
    if (!valor) return;

    const f = CONFIG.financiamento;
    const slider = $("valorImovel");

    // Se o imóvel passar do teto do simulador, o teto é ampliado.
    // Sem isto, um imóvel de R$ 4.550.000 seria cortado para
    // R$ 3.000.000 em silêncio e a simulação sairia errada.
    if (valor > Number(slider.max)) slider.max = valor;
    if (valor < Number(slider.min)) slider.min = valor;

    slider.value = valor;

    // a entrada volta para o mínimo proporcional ao novo valor
    ajustarSliderEntrada();
    $("entrada").value = Math.round(valor * f.entradaMinimaPercent / 100);

    // nome do imóvel no subtítulo, se veio junto
    const nome = params.get("imovel");
    if (nome) {
      const el = $("subtituloPagina");
      el.textContent = "Simulação para " + nome + ". " + el.textContent;
    }
  }
